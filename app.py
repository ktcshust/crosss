...
def preprocess_text(text):
    ...
    """
    # Nếu text là chuỗi và bắt đầu bằng "*" thì lưu lại prefix và xử lý phần còn lại
    if isinstance(text, str) and text.startswith("*"):
        ...
        prefix = ""
    text = text.replace("'", "")
    # Chỉ loại bỏ các ký tự không phải chữ, số hoặc khoảng trắng
...
def match_title_with_context(title_xlsx: str, full_xlsx: str) -> dict:
    ...
    for current in title_ranges:
        ...
        if curr_range_str in title_value_map and title_value_map[curr_range_str].strip().startswith("*"):
            ...
            print(f"Special title: {curr_range_str}")  # Debug: in ra các title đặc biệt
            continue
