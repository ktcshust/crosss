...
def match_fields(api_url: str, xlsx_file: str) -> dict:
    """
    So khớp các trường giữa API và file Excel.
    
    Đầu vào:
        - api_url: đường dẫn tới API trả về danh sách các field.
        - xlsx_file: đường dẫn tới file Excel.
        
    Trả về:
        - Dictionary với key là trường từ Excel và value là trường tương ứng trong API được khớp tốt nhất.
    """
    # Lấy danh sách các trường từ API và xử lý
    fields_database = get_json_fields_from_url(api_url)
    fields_database = [process_field_name(field) for field in fields_database]

    # Lấy danh sách các trường từ Excel và xử lý
    fields_excel = Extract_excel_data(xlsx_file)[1]
    ...
    return best_match_field_dict

# Ví dụ sử dụng:
if __name__ == "__main__":
    api_url = "https://crossreach-api-dev.mystg-env.com/ai-data?order_id=1"
    xlsx_file = "invoice_empty.xlsx"
    best_matches = match_fields(api_url, xlsx_file)
    print(best_matches)
